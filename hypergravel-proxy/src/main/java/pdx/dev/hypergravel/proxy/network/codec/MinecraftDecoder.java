...
public final class MinecraftDecoder extends MessageToMessageDecoder<ByteBuf> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) {
        ...
        if (packet instanceof pdx.dev.hypergravel.proxy.protocol.packet.SharedPackets.Disconnect d
                && state != pdx.dev.hypergravel.proxy.protocol.ProtocolState.LOGIN) {
    ...
    private static void release(Packet packet) {
        if (packet instanceof pdx.dev.hypergravel.proxy.protocol.packet.SharedPackets.PluginMessage pm) {
            pm.release();
        } else if (packet instanceof pdx.dev.hypergravel.proxy.protocol.packet.PlayPackets.JoinGame join) {
            join.release();
        }
    }
}
