...
public final class BackendPlaySessionHandler implements SessionHandler {
    ...
    public boolean handle(pdx.dev.hypergravel.tab.TabPackets.HeaderFooter packet) {
        ...
        return proxy.config().tab().enabled();
    }
    ...
    public boolean handle(SharedPackets.PluginMessage packet) {
        ...
        if (media.gitm.hypergravel.proxy.network.ServerBrand.CHANNEL.equals(channel)) {
            if (player.connection().active()) {
        ...
        if (media.gitm.hypergravel.proxy.network.BungeeCordChannel.isBungeeChannel(channel)) {
            media.gitm.hypergravel.proxy.network.BungeeCordChannel.handle(
